...
9. Fast-forward `mathieu` to the tested update branch, push normally, and create
   a permanent tag such as `0.80.6-mathieu.5`. Never force-push `mathieu`.
...
- The curated baseline includes `@hypabolic/pi-hypa@0.1.10` and hardened
  `pi-web-access@0.13.0`. Changing either extension or adding another requires
  exact dependency pins, audit documentation, and a new tested release tag.
- Every build must run `scripts/prepare-mathieu-extensions.mjs`. Its source and
  asset hash checks must fail closed when an audited artifact changes. Never
  weaken or bypass those checks to make an extension update build.
